...
We implement the model in AnyLogic as three interacting agent types. This section covers the simulation environment and the Victim and Main agents, before turning to the UAV agent that carries the search behavior.

\subsection{Simulation Environment}

The simulation environment is a two-dimensional continuous space of $45 \times 25$ meters. We overlay it with a shared pheromone grid of $90 \times 50$ cells, each covering a $0.5 \times 0.5$ meter area, which is the primary medium of stigmergic coordination. By default, the simulation runs two UAV agents and five Victim agents. A charging station sits at the bottom right of the search area.

\subsection{Victim Agent}

The Victim agent is a passive data container. Each instance holds a position assigned at initialization by the Main agent, plus three state variables: a flag indicating whether the victim has been found, the index of the discovering UAV, and the time of discovery. All victim logic runs in the Main agent, while the UAV handles sensing and confirmation.

\subsection{Main Agent}

The Main agent orchestrates the simulation. At initialization it seeds the random number generator for reproducibility, computes an evenly-spaced dispersal position for each UAV, and places victims by sampling the configured GMM. It also owns the shared pheromone grid, whose deposits decay by an exponential model:
\begin{equation}
    \tau(t) = \tau_0 \cdot e^{-\rho\,(t - t_0)},
    \label{eq:evaporation}
\end{equation}
where $\tau_0$ is the level deposited at reference time $t_0$ and $\rho$ is the evaporation rate. Each cell stores only its last value and deposit time, and the effective level is computed on demand from Equation~\eqref{eq:evaporation}, with no background evaporation event.

When a UAV confirms a victim, the Main agent marks the nearest Victim found, clears pheromone around the location, and logs the event to a comma-separated values (CSV) file. Once all victims are found it records the convergence time and stops, and a safety-stop event logs a timeout for any run that exceeds a time limit.
...
\begin{figure*}[ht]
    \centering
    \includegraphics[width=0.8\textwidth]{img/simulation.png}
